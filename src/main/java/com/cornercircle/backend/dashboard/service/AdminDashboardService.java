...
@Service public class AdminDashboardService {
 ...
 @Transactional(readOnly=true) public AdminDashboardResponse get(){
  ...
  var allEvents=events.findAllByOrderByCreatedAtDesc();
  ...
  var attention=new ArrayList<AdminDashboardResponse.AttentionItem>();
  long incomplete=allEvents.stream().filter(item->item.getPublicationStatus()==EventPublicationStatus.PUBLISHED).filter(this::isIncomplete).count();
  if(incomplete>0)attention.add(new AdminDashboardResponse.AttentionItem("event-details","EVENT_DETAILS",incomplete,incomplete+(incomplete==1?" event needs details":" events need details"),"Add missing date, time, location, capacity, or description.","/admin/events","Update events"));
  ...
  if(expiring>0)attention.add(new AdminDashboardResponse.AttentionItem("membership-expiry","MEMBERSHIP_EXPIRY",expiring,expiring+(expiring==1?" membership expires soon":" memberships expire soon"),"Memberships ending within the next 60 days.","/admin/members","View members"));
  ...
  if(nearlyFull>0)attention.add(new AdminDashboardResponse.AttentionItem("event-capacity","EVENT_CAPACITY",nearlyFull,nearlyFull+(nearlyFull==1?" event is nearly full":" events are nearly full"),"At least 80% of available places are reserved.","/admin/event-registrations","View registrations"));
