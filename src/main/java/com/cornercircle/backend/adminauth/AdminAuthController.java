...
public class AdminAuthController {
    ...
    @PostMapping("/login") public ResponseEntity<Void> login(@RequestHeader(value="X-Admin-Auth-Token",required=false)String token,@RequestBody Login request){authorize(token);return service.authenticate(request.username(),request.password())?ResponseEntity.noContent().build():ResponseEntity.status(401).build();}
    ...
    private void authorize(String supplied){byte[]a=serverToken.getBytes(StandardCharsets.UTF_8),b=supplied==null?new byte[0]:supplied.getBytes(StandardCharsets.UTF_8);if(serverToken.length()<32||!MessageDigest.isEqual(a,b))throw new org.springframework.web.server.ResponseStatusException(HttpStatus.UNAUTHORIZED);}
